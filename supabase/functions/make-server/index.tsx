...
// Create new user
app.post("/make-server-04d82a51/users", async (c) => {
  try {
    const body = await c.req.json();
    const userId = body.id || `U${Date.now().toString().slice(-6)}`;
    
    // Save the user data to the KV store
    ...
    // Also save to a list of users for quick access
...
// Get user by ID
app.get("/make-server-04d82a51/users/:id", async (c) => {
  try {
...
// Update user
app.put("/make-server-04d82a51/users/:id", async (c) => {
  try {
    const userId = c.req.param("id");
    const body = await c.req.json();
    ...
    const updatedUser = {
      ...
      lastLogin: body.lastLogin || user.lastLogin,
      updatedAt: new Date().toISOString(),
    };
...
// Delete user
app.delete("/make-server-04d82a51/users/:id", async (c) => {
  try {
    const userId = c.req.param("id");
    ...
    // Remove from list
    const usersList = await kv.get(`users:list`) || [];
    const filteredUsers = usersList.filter((u: any) => u.id !== userId);
